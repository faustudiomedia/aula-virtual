...
export default function QuizPlayer({ quiz, courseId, previousAttempt }: Props) {
  ...
  if (phase === 'intro') {
    return (
      <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-[#050F1F] mb-2">{quiz.title}</h2>
        ...
        <button
          ...
          className="px-8 py-3 rounded-xl bg-[#1A56DB] text-white font-semibold hover:opacity-90 transition shadow-lg shadow-[#1A56DB]/20"
        >
          Comenzar quiz →
        </button>
      </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className={`rounded-2xl p-8 text-center ${
          ...
          <p className="text-lg font-semibold text-[#050F1F] mb-1">
        ...
        <div className="space-y-4">
          <h2 className="text-base font-semibold text-[#050F1F]">Revisión de respuestas</h2>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-2 mb-3">
                  ...
                  <p className="text-sm font-semibold text-[#050F1F]">{q.question}</p>
                </div>
        ...
        <div className="flex gap-3">
          ...
          <Link
            ...
            className="flex-1 py-3 rounded-xl bg-[#1A56DB] text-white text-sm font-semibold hover:opacity-90 transition text-center"
          >
            Volver al curso
          </Link>
        </div>
  ...
  return (
    <div className="space-y-4">
      {questions.map((q, qIdx) => (
        <div key={q.id} className="bg-white rounded-2xl border border-black/5 shadow-sm p-5">
          <div className="flex items-start gap-3 mb-4">
            <span className="w-7 h-7 rounded-lg bg-[#EFF6FF] text-[#1A56DB] text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
              {qIdx + 1}
            </span>
            <p className="font-semibold text-[#050F1F] text-sm leading-relaxed">{q.question}</p>
          </div>
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl text-sm border-2 transition-all font-medium ${
                    selected
                      ...
                      : 'border-black/5 hover:border-[#BAE6FD] text-[#050F1F]/70 hover:text-[#050F1F]'
                  }`}
                >
                  <span className="inline-flex items-center gap-2">
                    <span className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                      selected ? 'border-[#1A56DB] bg-[#1A56DB]' : 'border-black/20'
                    }`}>
      ...
      {/* Progress indicator */}
      <div className="text-center text-xs text-[#050F1F]/40">
      ...
      <div className="flex gap-3">
        <button
          ...
          className="px-4 py-3 rounded-xl border border-black/10 text-sm font-semibold text-[#050F1F]/70 hover:bg-black/5 transition"
        >
          ← Volver
        </button>
        <button
          ...
          className="flex-1 py-3 rounded-xl bg-[#1A56DB] text-white text-sm font-semibold hover:opacity-90 transition disabled:opacity-50 shadow-lg shadow-[#1A56DB]/20"
        >
