...
export default function CourseForm() {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium text-[#050F1F] mb-1.5">Descripción</label>
      ...
      <div>
        <label className="block text-sm font-medium text-[#050F1F] mb-1.5">Horario de cátedra</label>
        <input name="schedule" placeholder="Ej: Lunes y Miércoles 18:00 - 20:00"
          className="w-full px-4 py-2.5 rounded-xl border border-black/10 text-[#050F1F] text-sm focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-transparent transition-all" />
        <p className="text-xs text-[#050F1F]/40 mt-1">Opcional. Días y horarios de la materia.</p>
      </div>

      <div className="flex items-center gap-3">
        <input type="checkbox" id="published" name="published" className="w-4 h-4 rounded accent-[#1A56DB]" />
        <label htmlFor="published" className="text-sm font-medium text-[#050F1F]">Publicar inmediatamente</label>
      </div>

      <div className="flex gap-3 pt-2">
        <SubmitButton label="Crear curso" loadingLabel="Creando..." />
        <a href="/dashboard/teacher"
          className="flex-1 py-2.5 flex items-center justify-center rounded-xl border border-black/10 text-[#050F1F]/70 font-semibold text-sm text-center hover:bg-black/5 transition-all">
          Cancelar
        </a>
      </div>
    </form>
