...
export async function login(prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (error) {
    const attempt = loginAttempts.get(parsed.data.email) || { count: 0, resetAt: Date.now() + LOCKOUT_MS };
    attempt.count += 1;
    loginAttempts.set(parsed.data.email, attempt);
    return { error: error.message === 'Invalid login credentials' ? 'Credenciales inválidas' : error.message };
  }
