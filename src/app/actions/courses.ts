...
export async function addMaterial(courseId: string, formData: FormData): Promise<ActionResult> {
  try {
    const supabase = await createClient()
    const user = await getAuthenticatedUser(supabase)
    if (!user) return { success: false, error: 'No autenticado' }
...
export async function updateMaterial(materialId: string, formData: FormData): Promise<ActionResult> {
  try {
    const supabase = await createClient()
    const user = await getAuthenticatedUser(supabase)
    if (!user) return { success: false, error: 'No autenticado' }
...
export async function createInstitute(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const slug = (formData.get('slug') as string)?.toLowerCase().replace(/\s+/g, '-').trim()
