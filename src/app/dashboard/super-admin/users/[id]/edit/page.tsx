...
export default async function SuperAdminEditUserPage({ params, searchParams }: Props) {
  ...
  if (!target) redirect('/dashboard/super-admin/users')
  ...
  async function handleUpdate(formData: FormData) {
    ...
    if (!result.success) {
      redirect(
        `/dashboard/super-admin/users/${id}/edit?error=${encodeURIComponent(result.error ?? 'Error desconocido')}`,
      )
    }
      ...
      <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-6">
        ...
        <form action={handleUpdate} className="space-y-5">
          ...
          <div>
            ...
            <select
              name="institute_id"
              defaultValue={target.institute_id ?? ''}
              className="w-full px-4 py-2.5 rounded-xl border border-black/10 text-sm text-[#050F1F] bg-white focus:outline-none focus:ring-2 focus:ring-[#38BDF8] transition-all"
            >
