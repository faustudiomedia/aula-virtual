  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm text-[var(--ag-text-muted)] mb-6">
        <Link href="/dashboard/teacher" className="hover:text-[var(--ag-navy)] transition-colors">Mis cursos</Link>
        <span>/</span>
        <Link href={`/dashboard/teacher/courses/${courseId}/materials`} className="hover:text-[var(--ag-navy)] transition-colors truncate max-w-[200px]">
          {course.title}
        </Link>
        <span>/</span>
        <span className="text-[var(--ag-text)] font-medium">Quizzes</span>
      </div>

      {/* Header */}
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[var(--ag-text)]">Quizzes</h1>
          <p className="text-[var(--ag-text-muted)] mt-1">{quizList.length} quiz{quizList.length !== 1 ? 'zes' : ''} en este curso</p>
        </div>
        <Link
          href={`/dashboard/teacher/courses/${courseId}/quizzes/new`}
          className="px-4 py-2.5 rounded-xl bg-[var(--ag-navy)] text-white text-sm font-semibold hover:opacity-90 transition shadow-lg "
        >
          + Nuevo quiz
        </Link>
      </div>
      ...
      {quizList.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-[var(--ag-border-light)] p-12 text-center">
          ...
          <p className="text-[var(--ag-text-muted)] mb-4">No hay quizzes en este curso todavía.</p>
          <Link
            href={`/dashboard/teacher/courses/${courseId}/quizzes/new`}
            className="inline-block px-4 py-2 rounded-xl bg-[var(--ag-navy)] text-white text-sm font-semibold hover:opacity-90 transition"
          >
            Crear el primero
          </Link>
        </div>
              ...
              >
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-[var(--ag-text)] truncate">{quiz.title}</p>
