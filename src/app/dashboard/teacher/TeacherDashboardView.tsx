...
export default function TeacherDashboardView({ teacherId, teacherName }: Props) {
    ...
    return (
      <div className="p-8 max-w-5xl mx-auto">
        ...
        <div className="grid gap-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm animate-pulse h-24" />
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Cursos activos', value: courses.filter((c) => c.published).length, color: '#1A56DB', bg: '#EFF6FF' },
          { label: 'Cursos totales', value: courses.length, color: '#7C3AED', bg: '#F5F3FF' },
          { label: 'Alumnos inscriptos', value: totalStudents, color: '#059669', bg: '#ECFDF5' },
        ].map((stat) => (
      ...
      {courses.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-[#BAE6FD] p-12 text-center">
          ...
          <p className="text-[#050F1F]/50 mb-4">Todavía no creaste ningún curso.</p>
          <Link href="/dashboard/teacher/courses/new"
