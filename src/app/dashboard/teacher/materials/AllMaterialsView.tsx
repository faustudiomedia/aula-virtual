...
export default function AllMaterialsView({ teacherId }: Props) {
  const { data: courses = [], isLoading: loadingCourses } = useTeacherCourses(teacherId)
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white rounded-xl border border-black/5 p-4 shadow-sm animate-pulse h-16" />
          ))}
        </div>
      ) : materials.length === 0 ? (
