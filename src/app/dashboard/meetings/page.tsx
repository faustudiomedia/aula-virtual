...
export default async function MeetingsPage() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-3xl mx-auto">
      ...
      <p className="text-[#050F1F]/50 mb-6">Videollamadas en tiempo real.</p>

      {/* Create form — teachers only */}
      {isTeacher && (
        <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-4 sm:p-5 mb-6">
          ...
          <form action={createMeeting} className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
              ...
              <button
                type="submit"
                className="px-5 py-3 rounded-xl bg-[#1A56DB] text-white text-sm font-semibold hover:bg-[#1A56DB]/90 transition-all whitespace-nowrap"
              >
                Crear sala
              </button>
            </div>
...
function MeetingRow({ m, host, userId, isTeacher, status }: MeetingRowProps) {
  ...
  return (
    <div className={`bg-white rounded-2xl border border-black/5 shadow-sm p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-3 ${opacity}`}>
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-base font-semibold text-[#050F1F] truncate">{m.display_name}</p>
