...
export default async function StudentAttendancePage({ params }: Props) {
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="mb-4">
        <Link href={`/dashboard/student/courses/${courseId}`} className="text-sm text-[#050F1F]/50 hover:text-[#050F1F] transition-colors">
          ← {course.title}
        </Link>
      </div>
      <h1 className="text-2xl font-bold text-[#050F1F] mb-6">Mi asistencia</h1>

      {/* Summary cards */}
      {total > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Asistencia', value: attendancePct !== null ? `${attendancePct}%` : '—', color: 'text-[#1A56DB]' },
            ...
            <div key={card.label} className="bg-white rounded-2xl border border-black/5 shadow-sm p-4 text-center">
              ...
              <p className="text-xs text-[#050F1F]/50 mt-1">{card.label}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden">
        {sessionList.length === 0 ? (
          <p className="p-6 text-sm text-[#050F1F]/40 text-center">El docente aún no registró clases.</p>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-black/5">
                <th className="text-left px-6 py-3 text-xs font-semibold text-[#050F1F]/50 uppercase tracking-wider">Fecha</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-[#050F1F]/50 uppercase tracking-wider">Tema</th>
                <th className="text-center px-6 py-3 text-xs font-semibold text-[#050F1F]/50 uppercase tracking-wider">Estado</th>
              </tr>
            </thead>
              ...
              {sessionList.map(s => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-black/[0.015]">
                    <td className="px-6 py-3 text-sm text-[#050F1F] capitalize">{formatDate(s.session_date)}</td>
                    <td className="px-6 py-3 text-sm text-[#050F1F]/60">{s.topic ?? '—'}</td>
