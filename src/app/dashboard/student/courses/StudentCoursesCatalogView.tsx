...
function CatalogCard({ courseId, title, description, isEnrolled, defaultIdx }: CatalogCardProps) {
  ...
  const inner = (
    <div className="group bg-white rounded-xl overflow-visible relative transition-all duration-200"
      ...
      {/* Card header */}
      <div className="h-20 flex items-end px-4 pb-3 relative rounded-t-xl overflow-hidden"
        ...
        {/* Status badge */}
        <span className="relative z-10 text-[11px] font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1.5"
          style={{
      ...
      <div className="p-4">
        ...
        {isEnrolled ? (
          <div className="w-full py-2 rounded-lg text-white text-sm font-semibold text-center transition-all"
...
export default function StudentCoursesCatalogView({ instituteId, userId }: Props) {
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      {/* ── Search ── */}
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.1 }} className="relative mb-8">
        <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2"
          style={{ color: "var(--ag-text-muted)" }} />
        <input
          type="text" id="course-search" value={search}
      ...
      {/* ── Empty states ── */}
      {courses.length === 0 ? (
        <div className="rounded-xl p-16 text-center bg-white"
        ...
        <div className="rounded-xl p-10 text-center bg-white"
          style={{ border: "2px dashed var(--ag-border-light)" }}>
          <p className="text-sm" style={{ color: "var(--ag-text-muted)" }}>
