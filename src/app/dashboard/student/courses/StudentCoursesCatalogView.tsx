    ...
    return (
      <div className="p-8 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm animate-pulse h-40" />
          ))}
        </div>
      </div>
    )
      ...
      {courses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {courses.map((course) => {
            ...
            return (
              <div key={course.id} className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm hover:shadow-md transition-shadow">
