      ...
      {enrollments && enrollments.length > 0 && (
        <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-6">
          ...
          <div className="space-y-3">
            {[
              { label: 'Sin iniciar (0%)', filter: (p: number) => p === 0, color: '#E5E7EB' },
              { label: 'Iniciado (1–33%)', filter: (p: number) => p >= 1 && p <= 33, color: '#FCA5A5' },
              { label: 'En progreso (34–66%)', filter: (p: number) => p >= 34 && p <= 66, color: '#38BDF8' },
              { label: 'Avanzado (67–99%)', filter: (p: number) => p >= 67 && p <= 99, color: '#1A56DB' },
              { label: 'Completado (100%)', filter: (p: number) => p === 100, color: '#059669' },
            ].map((bucket) => {
              const count = enrollments.filter((e) => bucket.filter(e.progress)).length
              const pct = Math.round((count / enrollments.length) * 100)
              return (
                <div key={bucket.label} className="flex items-center gap-3">
                  <span className="text-xs text-[#050F1F]/50 w-44 flex-shrink-0">{bucket.label}</span>
                  ...
                  <span className="text-xs font-medium text-[#050F1F]/60 w-10 text-right">{count}</span>
                </div>
              )
            })}
          </div>
