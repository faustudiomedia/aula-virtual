...
export default async function InstituteDetailPage({ params }: Props) {
  ...
  const students = (members ?? []).filter((m: any) => m.role === 'alumno').length
  const teachers = (members ?? []).filter((m: any) => m.role === 'profesor').length
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <a href="/dashboard/admin" className="text-[#1A56DB] hover:underline text-sm">← Institutos</a>
      <h1 className="text-2xl font-bold text-[#050F1F] mt-3 mb-1">Editar instituto</h1>
      <p className="text-[#050F1F]/50 mb-6">Actualizá los datos y el branding del instituto.</p>
      ...
      {/* Courses list */}
      <h2 className="text-base font-semibold text-[#050F1F] mb-3">Cursos del instituto</h2>
      {courseList.length === 0 ? (
        <p className="text-sm text-[#050F1F]/40">Sin cursos.</p>
      ) : (
        <div className="space-y-2">
          {courseList.map((c: any) => (
            <div key={c.id} className="bg-white rounded-xl border border-black/5 px-4 py-3 flex items-center justify-between text-sm">
