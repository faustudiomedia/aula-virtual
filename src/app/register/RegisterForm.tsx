...
const inputClass =
  "w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-transparent transition-all text-sm";
...
export default function RegisterForm({ institutes }: { institutes: Institute[] }) {
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div>
        <label htmlFor="full_name" className="block text-sm font-medium text-[#BAE6FD] mb-1.5">
          Nombre completo
        </label>
      ...
      <div>
        <label htmlFor="institute_id" className="block text-sm font-medium text-[#BAE6FD] mb-1.5">
          Instituto
        </label>
        <select
          id="institute_id"
          ...
          required
          defaultValue=""
          className="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-transparent transition-all text-sm"
        >
          <option value="" disabled className="text-gray-400 bg-[#050F1F]">Seleccioná tu instituto</option>
          {institutes.map((inst) => (
            <option key={inst.id} value={inst.id} className="bg-[#050F1F]">
              {inst.name}
            </option>
          ))}
        </select>
      ...
      <div className="text-center pt-2">
        <Link href="/login" className="text-sm text-[#BAE6FD]/60 hover:text-[#BAE6FD] transition">
