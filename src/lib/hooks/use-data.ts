...
export const queryKeys = {
  ...
  enrollments: (studentId: string) => ['enrollments', 'student', studentId] as const,
  courseEnrollments: (courseId: string) => ['enrollments', 'course', courseId] as const,
  enrollmentCounts: (courseIds: string[]) => ['enrollments', 'counts', courseIds] as const,
...
export function useCourses(options?: CoursesOptions) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from('courses')
        .select('*, profiles(full_name, email), institutes(name)', { count: 'exact' })
        .order('created_at', { ascending: false })
